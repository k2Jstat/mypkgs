...
class KL_Divergence():
    ...
    def run(self):
        ...
        kl_df.sort_values("KL_Divergence",ascending = False,inplace = True)
        kl_df.reset_index(drop = True,inplace = True)
        ...
        tbl_df2 = kl_df[["columns","KL_Divergence"]].merge(tbl_df,on = "columns")
    ...
    def plot(self,yoko = 4):
        ...
        col_list = kl_df["columns"].to_list()
        ...
        for num,col_ in enumerate(col_list):
            try:
                i,j = num // yoko,num % yoko  
                temp = tbl_df[tbl_df["columns"] == col_].drop(columns = ["columns","KL_Divergence"]).set_index("bins")        
                temp = temp * 100
                kld = kl_df[kl_df["columns"] == col_]["KL_Divergence"].values[0]

                if tate == 1:
                    temp.plot(kind = "bar",grid = True,title = col_ + " : " + str(kld),ec = "black",ax = ax[j])
                    ax[j].yaxis.set_major_formatter(mtick.PercentFormatter())

                else :
                    temp.plot(kind = "bar",grid = True,title = col_ + " : " + str(kld),ec = "black",ax = ax[i][j])
